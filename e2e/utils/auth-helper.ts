...
export class AuthHelper {
  ...
  async loginAsTenant(
    ...
    const { email, password } = credentials || DEFAULT_CREDENTIALS.tenant;
    const waitForRedirect = options?.waitForRedirect ?? true;

    await this.page.goto('/tenant/auth/login');
    await this.page.waitForLoadState('networkidle');
    ...
    if (waitForRedirect) {
      ...
      await expect(this.page).toHaveURL(/\/tenant/, { timeout: 15000 });
    }
  }
  ...
  async ensureLoggedIn(
    credentials?: LoginCredentials,
    role: 'user' | 'admin' | 'tenant' | 'superAdmin' = 'user'
  ): Promise<void> {
    if (await this.isLoggedIn()) {
      return;
    }
    ...
    switch (role) {
      case 'admin':
        ...
        break;
      case 'tenant':
        await this.loginAsTenant(credentials);
        break;
      case 'superAdmin':
