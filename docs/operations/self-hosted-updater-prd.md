...
- modern release trains: build once, validate on `candidate`, and only then promote the same artifact to `stable`.
...
- Small runtime or catalog fixes can be verified quickly without minting a new merchant-visible stable release every time.
...
13. Self-hosted updates must distinguish `candidate` validation from `stable` merchant availability.
14. Promotion from `candidate` to `stable` must reuse the already-built runtime artifacts instead of rebuilding images.
15. Official theme/admin marketplace completeness must be part of release validation for self-hosted admin, so a stale official catalog seed cannot silently hide most official themes.
...
- Operators understand that `candidate` is the fast validation lane and `stable` is the merchant-visible lane.
...
- Release operations docs explain when to cut a new stable versus when to validate the same artifact on the candidate lane.
