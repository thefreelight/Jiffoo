...
- Build runtime images once and promote the same verified artifact from `candidate` to `stable` instead of rebuilding for every validation pass.
...
- The public self-hosted feed must expose at least two channels:
  - `candidate` for fast validation on canary tenants or operator-run staging hosts
  - `stable` for merchant-visible upgrades
- Promotion from `candidate` to `stable` must update feed metadata, not rebuild the runtime images.
- Self-hosted release metadata must converge across image tag, embedded package version, and persisted upgrade state before a release can be promoted to `stable`.
- The self-hosted admin surface must not regress official catalog completeness because a release branch carries an outdated official launch seed.
...
- Candidate validation can catch runtime/version drift or stale official catalog seeds without forcing every fix through a new stable merchant-visible release.
