...
export async function fetchActiveTheme(): Promise<ActiveTheme | null> {
  try {
    ...
    const { applyActiveThemeDomainOverride } = await import('@/lib/storefront-domain-overrides');
    ...
    return applyActiveThemeDomainOverride((response.data || null) as ActiveTheme | null, hostname);
  } catch (error) {
...
/**
 * Get runtime JS URL for a theme when the pack ships a storefront bundle.
 */
