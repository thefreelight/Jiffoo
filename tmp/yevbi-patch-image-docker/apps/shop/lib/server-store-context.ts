import { buildServerApiUrl, resolvePublicOrigin } from './server-api-url';
import { applyStorefrontDomainOverride } from './storefront-domain-overrides';
...
export async function getServerStoreContext(
  ...
  try {
    ...
    if (data.success && data.data) {
      const context = data.data as ServerStoreContext;
      const publicOrigin = await resolvePublicOrigin();
      return applyStorefrontDomainOverride(context, new URL(publicOrigin).hostname);
    }
