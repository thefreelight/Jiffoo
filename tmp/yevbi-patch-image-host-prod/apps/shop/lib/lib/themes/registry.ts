...
export const BUILTIN_THEMES: ThemeRegistry = {
  ...
  // Official embedded runtime entries. `esim-mall` is the generic eSIM
  // storefront renderer; `yevbi` remains a legacy migration bridge only.
  'esim-mall': {
    meta: {
      slug: 'esim-mall',
      name: 'eSIM Mall Theme',
      version: '1.1.0',
      description: 'Premium editorial storefront theme for international eSIM marketplaces.',
      category: 'esim',
      author: 'Jiffoo',
      target: 'shop',
      tags: ['esim', 'travel', 'connectivity', 'premium', 'editorial'],
    },
  ...
  'yevbi': {
    ...
    load: async () => {
      const module = await import('@shop-themes/yevbi/src/runtime');
      return module.default || module.theme;
    },
  },
};
