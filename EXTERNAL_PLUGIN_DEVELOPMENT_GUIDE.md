...
## 10. Licensing & Commercial Distribution

### Three-Layer License Boundary

Jiffoo's open-source ecosystem uses a three-layer license model:

| Layer | What | License | Can be closed-source? |
|-------|------|---------|----------------------|
| GPL Core | `apps/api`, `apps/shop`, `apps/admin`, `packages/shared` | GPL-2.0+ | ❌ No |
| MIT SDKs | `@jiffoo/plugin-sdk`, `@jiffoo/theme-api-sdk`, `@jiffoo/core-api-sdk`, `@jiffoo/ui` | MIT | ✅ Yes (SDK itself is MIT) |
| External-HTTP Plugins | Your plugin (separate process) | Any | ✅ Yes |

### Compliant Path for Closed-Source Plugins

To build a **proprietary/closed-source plugin** for Jiffoo:

1. **Use external-http runtime**: Your plugin runs as a separate HTTP service (any language, any license)
2. **Use MIT SDKs**: Import `@jiffoo/plugin-sdk` for types and utilities — MIT licensed, no copyleft
3. **Communicate via HTTP only**: Use the Plugin Gateway HTTP protocol (documented in §3)
4. **Do not import GPL code**: Never import or link against `apps/api`, `packages/shared`, or other GPL packages

### Non-Compliant Path (Internal-Fastify)

If your plugin runs **inside the Jiffoo API process** (internal-fastify runtime):
- It is a **derivative work** of the GPL core
- You **must** distribute it under GPL-2.0-or-later
- Third-party internal-fastify plugins are rejected unless signed by Jiffoo (official trust level)

### Interface Exceptions

The following protocol contracts are designated as stable interfaces that may be implemented without triggering copyleft:

1. **Plugin Gateway HTTP Protocol** — this document, §3
2. **Webhook Protocol** — outbound webhook payload schema
3. **Theme Pack Declarative Format** — `theme.json` + template JSON schema

Implementing these protocols in a separate process does not make your work derivative of the GPL core.

### Full Statement

See [LICENSE-EXCEPTIONS.md](./LICENSE-EXCEPTIONS.md) for the complete license boundary statement.


## 11. Plugin Trust Levels & Signature (Platform Evolution 2026 H2)

### 11.1 Trust Level Model

All plugins are assigned a trust level upon installation:

| Trust Level | How Obtained | Allowed Runtime |
|-------------|-------------|-----------------|
| `builtin` | Ships with Jiffoo distribution | `internal-fastify` |
| `official` | Signed by a trusted Ed25519 key | `internal-fastify` |
| `third-party` | Unsigned or signed by unknown key | `external-http` only |

**Third-party plugins cannot use `internal-fastify` runtime.** If you are building a closed-source or unsigned plugin, you must use `external-http` runtime (see §2).

### 11.2 Package Signature Format

Official plugins are distributed as ZIP packages containing:

```
your-plugin.zip
├── manifest.json    ← plugin metadata + file hash list
├── manifest.sig     ← Ed25519 detached signature of manifest.json (base64)
├── server/
│   └── index.js
└── package.json
```

**`manifest.json` (signed manifest format):**
```json
{
  "schemaVersion": 1,
  "slug": "your-plugin",
  "version": "1.0.0",
  "files": {
    "server/index.js": "sha256-hex-of-file-content",
    "package.json": "sha256-hex-of-file-content"
  }
}
```

**Verification process:**
1. Platform loads `manifest.json` and `manifest.sig` from the ZIP
2. Tries each trusted public key to verify the Ed25519 signature
3. If verified, checks each file's SHA-256 against the manifest's `files` map
4. If any file hash mismatches, installation is rejected

### 11.3 Trusted Keys

The platform trusts public keys from two sources:

1. **Built-in official key** — bundled with the platform (`keys/official.pub`)
2. **Operator-configured keys** — via `JIFFOO_EXTRA_TRUSTED_KEYS` environment variable (newline-separated PEM public keys)

If you are an enterprise customer and need to sign internal-fastify plugins with your own key, contact Jiffoo support to have your public key added to the trusted key set.

### 11.4 Grace Period for Existing Plugins

Plugins installed before the trust level enforcement was introduced continue to work, but:

- A warning is logged at startup
- The Admin plugin center displays a yellow warning banner
- Starting from `v1.2.0`, third-party internal-fastify plugins will be **disabled** at startup

To migrate: either obtain an official signature, or switch to `external-http` runtime.
