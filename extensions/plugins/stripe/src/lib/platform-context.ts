...
export function getContext(headers: Record<string, string | string[] | undefined>): PluginContext {
  return {
    ...
    installationId: getHeaderValue(headers, 'x-installation-id') || 'default',
...
/**
 * Decode x-plugin-config header (base64url-encoded JSON).
 */
export function getPluginConfig(headers: Record<string, string | string[] | undefined>): StripePluginConfig {
  const raw = getHeaderValue(headers, 'x-plugin-config');
  if (!raw) return {};
  try {
...
export function createContextMiddleware() {
  return (
    req: express.Request & { pluginContext?: PluginContext; pluginConfig?: StripePluginConfig },
    _res: express.Response,
    next: express.NextFunction
  ) => {
    req.pluginContext = getContext(req.headers as Record<string, string | string[] | undefined>);
    req.pluginConfig = getPluginConfig(req.headers as Record<string, string | string[] | undefined>);
    next();
  };
}
