...
| 层级 | 范围 | 许可证 | 可闭源？ |
|------|------|--------|---------|
| GPL 核心 | `apps/api`、`apps/shop`、`apps/admin`、`packages/shared` | GPL-2.0+ | ❌ |
| MIT SDK | `@jiffoo/plugin-sdk`、`@jiffoo/theme-api-sdk`、`@jiffoo/core-api-sdk` | MIT | ✅ |
| 外部 HTTP 插件 | 独立进程的第三方插件 | 任意 | ✅ |

### 内部插件的 GPL 约束

内部插件（internal-fastify 运行时）运行在 Jiffoo API 进程内，共享 Fastify 运行时与内存空间，属于 GPL 核心的衍生作品。分发内部插件**必须**以 GPL-2.0-or-later 许可证开源。

### 闭源插件的合规路径

如需分发闭源商业插件：
1. 使用 **external-http 运行时**（独立 HTTP 服务）
2. 使用 MIT SDK（`@jiffoo/plugin-sdk`）获取类型定义与工具
3. 仅通过 HTTP 协议与 Plugin Gateway 通信
4. **不要** import 或 link 任何 GPL 包的代码

详见 [LICENSE-EXCEPTIONS.md](../LICENSE-EXCEPTIONS.md)。
...
**核心规则：** `internal-fastify` 插件运行在平台同进程内，必须为 `builtin` 或 `official` 信任等级。`third-party` 插件只能使用 `external-http` 运行时。

**manifest 声明：** internal-fastify 插件的 `manifest.json` 必须声明 `trustLevel` 字段：
...
**签名验证：** `official` 信任等级的插件需通过 Ed25519 签名验证。ZIP 包内包含 `manifest.json`（含文件哈希清单）和 `manifest.sig`（分离签名）。可信公钥来源：
- 内置官方密钥 `keys/official.pub`
- 环境变量 `JIFFOO_EXTRA_TRUSTED_KEYS`（换行分隔的 PEM 公钥列表）

**宽限期：** 已安装的存量 `third-party` + `internal-fastify` 插件不会立即被禁用，启动时输出 warn 日志。强制执行版本为 `v1.2.0`。

**网关防护：** external-http 插件代理请求受以下保护：
- 请求超时：默认 10 秒（可按插件配置 `timeoutMs`，上限 60 秒）
- 响应大小上限：5 MB
- 熔断器：60s 窗口内失败率 ≥50% 且样本 ≥10 → 开启 30s
- 限流：60 req/min per plugin
