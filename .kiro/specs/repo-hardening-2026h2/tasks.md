...
- [x] 2.1 门禁 workflow 骨架 (R2.1, R2.2)
  ...
  - [x] 2.1.2 Job `static-checks`：pnpm 缓存 install → `pnpm type-check` → `pnpm lint`（turbo 并行）_(type-check 已上线且全仓归零（含 API 的 20 个存量类型错误清零）；lint 已于 2026-07-14 补齐：根 eslint.config.mjs（ESLint 9 扁平配置），基线 0 error / ~997 warning（正确性规则 error、风格规则 warn 的 ratchet 策略），CI static-checks 跑 `npx eslint .`；顺手修复 finally-return 真 bug（polling-worker）等 7 处 error)_
