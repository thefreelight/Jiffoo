...
import { buildResumeFactCandidates, detectResumeFormat, extractResumeDocumentText, RESUME_CONTENT_TYPES } from './resume-extraction';
...
export async function tryNativeRemoteRadarResumeDocuments(request: Request, env: Env): Promise<Response | null> {
  ...
  if (listMatch && request.method === 'POST') {
    ...
    if (file.size <= 0 || file.size > 8 * 1024 * 1024) return fail('FILE_TOO_LARGE', 'Resume files must be between 1 byte and 8 MB');
    const bytes = new Uint8Array(await file.arrayBuffer());
    // Accept by sniffed content (PDF/DOCX/ODT/RTF/HTML/TXT/Markdown), not the
    // browser-declared MIME, so .md/.rtf/.htm resumes are not wrongly rejected.
    const format = detectResumeFormat(bytes, file.type, file.name);
    if (!format) return fail('UNSUPPORTED_FILE_TYPE', 'Supported resume formats: PDF, DOCX, ODT, RTF, HTML, TXT, Markdown');
    const contentType = RESUME_CONTENT_TYPES[format];
    ...
    await env.DB.prepare(`INSERT INTO remoteradar_resume_documents (id,user_id,resume_id,object_key,filename,content_type,byte_size,extraction_status,created_at,updated_at) VALUES (?1,?2,?3,?4,?5,?6,?7,'pending',?8,?8)`).bind(id, user.id, resumeId, key, file.name.slice(0, 240), contentType, file.size, now).run();
    return Response.json({ success: true, data: item({ id, resume_id: resumeId, filename: file.name.slice(0, 240), content_type: contentType, byte_size: file.size, extraction_status: 'pending', created_at: now, updated_at: now }) }, { status: 201, headers: { 'cache-control': 'no-store' } });
  }
