...
describe('Upgrade Endpoints', () => {
  ...
  describe('GET /api/upgrade/version', () => {
    ...
    it('should prefer the public manifest when one is configured and reachable', async () => {
      ...
      vi.stubGlobal(
        ...
        vi.fn(async () => ({
          ...
          json: async () => ({
            ...
            deliveryMode: 'image-first',
            images: {
              api: 'registry.example.com/jiffoo-oss/api:1.0.23',
              admin: 'registry.example.com/jiffoo-oss/admin:1.0.23',
              shop: 'registry.example.com/jiffoo-oss/shop:1.0.23',
              updater: 'registry.example.com/jiffoo-oss/updater:1.0.23',
            },
            releaseDate: '2026-04-11T00:00:00.000Z',
            changelogUrl: 'https://example.com/changelog/1.0.23',
            minimumCompatibleVersion: '1.0.0',
            minimumAutoUpgradableVersion: '1.0.0',
            requiresManualIntervention: false,
      ...
      expect(response.statusCode).toBe(200);
      const body = response.json();
      expect(body.data.latestVersion).toBe('1.0.23');
    ...
    it('should normalize -opensource runtime versions before comparing against the public feed', async () => {
      ...
      expect(response.statusCode).toBe(200);
      const body = response.json();
      expect(body.data.currentVersion).toBe('1.0.23');
      expect(body.data.latestVersion).toBe('1.0.23');
