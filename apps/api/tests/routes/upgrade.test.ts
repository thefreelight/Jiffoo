...
describe('Upgrade Endpoints', () => {
  ...
  describe('GET /api/upgrade/version', () => {
    ...
    it('should prefer the public manifest when one is configured and reachable', async () => {
      ...
      vi.stubGlobal(
        ...
        vi.fn(async () => ({
          ...
          json: async () => ({
            ...
            deliveryMode: 'image-first',
            images: {
              api: 'registry.example.com/jiffoo-oss/api:1.0.19',
              admin: 'registry.example.com/jiffoo-oss/admin:1.0.19',
              shop: 'registry.example.com/jiffoo-oss/shop:1.0.19',
              updater: 'registry.example.com/jiffoo-oss/updater:1.0.19',
            },
            releaseDate: '2026-04-11T00:00:00.000Z',
            changelogUrl: 'https://example.com/changelog/1.0.19',
            minimumCompatibleVersion: '1.0.0',
            minimumAutoUpgradableVersion: '1.0.0',
            requiresManualIntervention: false,
      ...
      expect(response.statusCode).toBe(200);
      const body = response.json();
      expect(body.data.latestVersion).toBe('1.0.19');
    ...
    it('should strip the opensource distribution suffix from currentVersion and stop advertising a phantom update', async () => {
      ...
      expect(response.statusCode).toBe(200);
      const body = response.json();
      expect(body.data.currentVersion).toBe('1.0.19');
      expect(body.data.latestVersion).toBe('1.0.19');
