...
/**
 * Public plugin runtime gateway paths: /extensions/plugin/:slug/{health,manifest,api,api/*}
 *
 * These are intentionally NOT admin-protected (see extension-installer/routes.ts):
 * they proxy into each plugin's own internal Fastify runtime, which implements its
 * own auth as needed. Exempting them here so anonymous / Shop / Admin callers can
 * reach the runtime; real admin management routes (instances, install, purge, ...)
 * stay admin-protected because they don't match this shape.
 */
function isPublicPluginGatewayPath(pathname: string): boolean {
  return /^\/extensions\/plugin\/[^/]+\/(health|manifest|api)(\/.*)?$/.test(pathname);
}
...
export async function adminPermissionGuard(
  ...
  // Public plugin runtime gateway (health/manifest/api) is gated per-plugin, not by admin.
  if (isPublicPluginGatewayPath(pathname)) {
    return;
  }
