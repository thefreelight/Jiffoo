...
const userProfileSchema = {
  ...
  properties: {
    ...
    admin: {
      ...
      required: ['membershipId', 'role', 'status', 'permissions', 'isOwner', 'extraPermissions', 'revokedPermissions'],
    },
    adminRole: { type: 'string', description: 'Normalized admin role', nullable: true },
    permissions: {
      type: 'array',
      description: 'Granted admin permissions for the current user',
      items: { type: 'string' },
    },
    isOwner: { type: 'boolean', description: 'Whether the user is treated as owner-level admin' },
    adminStatus: { type: 'string', enum: ['ACTIVE', 'SUSPENDED'], nullable: true },
