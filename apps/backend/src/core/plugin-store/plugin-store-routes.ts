...
import { PluginStatus } from '../../../../../plugins/core/types';
...
export async function pluginStoreRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/plugins', {
    ...
    try {
      ...
      // 获取统一插件管理器来获取真实的插件状态
      const unifiedManager = paymentManager.getUnifiedPluginManager();

      // 更新插件状态为真实状态
      const pluginsWithRealStatus = await Promise.all(
        plugins.map(async (plugin) => {
          try {
            ...
            const realStatus = await (unifiedManager as any).getPluginStatus(plugin.id);

            // 将PluginStatus枚举转换为字符串状态
            let status: string;
            switch (realStatus) {
              case PluginStatus.UNINSTALLED:
                status = 'uninstalled';
                break;
      ...
      switch (validation.sort) {
        case 'name':
          pluginsWithRealStatus.sort((a, b) => a.displayName.localeCompare(b.displayName));
          break;
        case 'rating':
          pluginsWithRealStatus.sort((a, b) => b.stats.rating - a.stats.rating);
          break;
        case 'downloads':
          pluginsWithRealStatus.sort((a, b) => b.stats.downloads - a.stats.downloads);
          break;
        case 'updated':
          pluginsWithRealStatus.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
          break;
      }

      // 分页
      const total = pluginsWithRealStatus.length;
      ...
      const paginatedPlugins = pluginsWithRealStatus.slice(startIndex, endIndex);
