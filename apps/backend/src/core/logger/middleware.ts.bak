...
import { LoggerService, OperationType } from './logger';
...
// 错误日志中间件
export async function errorLogMiddleware(error: Error, request: FastifyRequest, reply: FastifyReply) {
...
// 认证日志中间件
export async function authLogMiddleware(request: FastifyRequest, reply: FastifyReply) {
  const user = (request as any).user;
...
// 搜索日志中间件
export async function searchLogMiddleware(request: FastifyRequest, reply: FastifyReply) {
