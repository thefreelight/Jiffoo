/**
 * Contact Page — server wrapper for page-level SEO metadata.
 */
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return generatePublicPageMetadata({ route: 'contact', locale, origin, brandName: ctx?.theme?.config?.brand?.name?.trim() || ctx?.storeName?.trim() });
}
