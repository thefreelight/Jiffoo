/**
 * New Arrivals Page for Shop Application
 *
 * Displays newly added products with sorting and pagination.
...
export default function NewArrivalsPage() {
  ...
  const loadProducts = React.useCallback(async (page = 1, filters: ProductSearchFilters = {}) => {
    try {
      ...
      const response = await ProductService.getProducts(page, 12, {
        ...filters,
        sortBy: sortBy === 'newest' ? 'createdAt' : sortBy as 'price' | 'name' | 'createdAt' | 'stock',
        sortOrder,
        locale: nav.locale,
      });
  ...
  if (!theme?.components?.NewArrivalsPage) {
    const NotFoundComponent = theme?.components?.NotFound;
    if (NotFoundComponent) {
      return (
        <NotFoundComponent
          route="/new-arrivals"
          message={getText('common.errors.componentUnavailable', 'New arrivals page component is not available')}
          config={config}
          locale={nav.locale}
          t={t}
          onGoHome={() => nav.push('/')}
    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-600">{getText('common.errors.componentUnavailable', 'Unable to load new arrivals page component')}</p>
        </div>
      </div>
    );
  }
  ...
  const NewArrivalsPageComponent = theme.components.NewArrivalsPage;
