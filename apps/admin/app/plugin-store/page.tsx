...
export default function PluginStorePage() {
  ...
  useEffect(() => {
    const loadPlugins = async () => {
      try {
        ...
        const response = await fetch('/api/commercial/plugins')
  ...
  const getStatusBadge = (plugin: Plugin) => {
    if (plugin.isLicensed) {
      return <Badge className="bg-green-100 text-green-800">Licensed</Badge>
    }
    if (plugin.isInstalled) {
      return <Badge className="bg-blue-100 text-blue-800">Demo Version</Badge>
    }
