    ...
    <div className="p-6">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Button className="bg-gray-900 hover:bg-gray-800">
            ...
            {getText('merchant.customers.addCustomer', 'Add Customer')}
          </Button>
        </div>
        {/* In-page Navigation */}
      ...
      {/* Stats Cards */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            ...
            <Card>
              <CardContent className="p-6">
                <div className="flex items-center">
                  <div className="flex-1">
                    <p className="text-sm font-medium text-gray-600">{getText('merchant.customers.vip', 'VIP')}</p>
                ...
                <div className="flex gap-3">
                  <Select value={selectedStatus} onValueChange={setSelectedStatus}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="VIP">{getText('merchant.customers.vip', 'VIP')}</SelectItem>
                      <SelectItem value="Inactive">{getText('merchant.customers.inactive', 'Inactive')}</SelectItem>
                    </SelectContent>
                  </Select>
                  <Button variant="outline">
                    <Filter className="w-4 h-4 mr-2" />
                    {getText('merchant.customers.moreFilters', 'More Filters')}
                  </Button>
                </div>
              </div>
            </CardContent>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-blue-900">
                  {selectedUsers.length} {getText('merchant.customers.usersSelected', 'user(s) selected')}
                </span>
                <div className="flex gap-2">
                  ...
                  >
                    {getText('merchant.customers.updateRole', 'Update Role')}
                  </Button>
                  <Button
                    variant="outline"
                    size="sm"
                  ...
                  >
                    {getText('merchant.customers.deleteSelected', 'Delete Selected')}
                  </Button>
                </div>
              </div>
            </div>
                          ...
                          <td className="py-4 px-6">
                            <div className="flex items-center space-x-2">
                              ...
                              <Button
                                variant="ghost"
                                size="sm"
                                title={getText('merchant.customers.editRole', 'Edit Role')}
                              ...
                              <Button
                                variant="ghost"
                                size="sm"
                                title={getText('merchant.customers.editUser', 'Edit User')}
                              ...
                              <Button
                                variant="ghost"
                                size="sm"
                                title={getText('merchant.customers.deleteUser', 'Delete User')}
        ...
        <Dialog open={!!editingUser} onOpenChange={(open) => !open && setEditingUser(null)}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{getText('merchant.customers.editUser', 'Edit User')}</DialogTitle>
              <DialogDescription>
                {getText('merchant.customers.editUserDesc', "Update user information. Click save when you're done.")}
              </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="email" className="text-right">
                  {getText('merchant.customers.email', 'Email')}
                </Label>
                <Input
                  id="email"
                  type="email"
                  ...
                  title={getText('merchant.customers.emailCannotBeModified', 'Email cannot be modified')}
                />
              </div>
            </div>
            <DialogFooter>
              ...
              <Button onClick={handleUpdateUser} disabled={updateUserMutation.isPending}>
                {updateUserMutation.isPending ? getText('merchant.customers.saving', 'Saving...') : getText('merchant.customers.saveChanges', 'Save changes')}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={!!deletingUserId} onOpenChange={(open) => !open && setDeletingUserId(null)}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{getText('merchant.customers.deleteUser', 'Delete User')}</DialogTitle>
              <DialogDescription>
                {getText('merchant.customers.deleteUserConfirm', 'Are you sure you want to delete this user? This action cannot be undone.')}
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
        ...
        <Dialog open={showBatchDialog} onOpenChange={setShowBatchDialog}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{getText('merchant.customers.batchOperation', 'Batch Operation')}</DialogTitle>
              <DialogDescription>
                {batchAction === 'updateRole' && getText('merchant.customers.updateRoleConfirm', 'Update role for {count} selected user(s)').replace('{count}', String(selectedUsers.length))}
                {batchAction === 'delete' && (
                  <span className="text-red-600">
                    {getText('merchant.customers.batchDeleteConfirm', 'Are you sure you want to delete {count} selected user(s)? This action cannot be undone.').replace('{count}', String(selectedUsers.length))}
                  </span>
                )}
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="grid gap-4 py-4">
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="role" className="text-right">
                    {getText('merchant.customers.role', 'Role')}
                  </Label>
                  <Select value={batchRole} onValueChange={(value: 'USER' | 'TENANT_ADMIN') => setBatchRole(value)}>
                    <SelectTrigger className="col-span-3">
                      <SelectValue placeholder={getText('merchant.customers.selectRole', 'Select role')} />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="USER">{getText('merchant.customers.roleUser', 'USER')}</SelectItem>
                      <SelectItem value="TENANT_ADMIN">{getText('merchant.customers.roleTenantAdmin', 'TENANT_ADMIN')}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
            ...
            <DialogFooter>
              ...
              >
                {batchOperationsMutation.isPending ? getText('merchant.customers.processing', 'Processing...') : getText('merchant.customers.confirm', 'Confirm')}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={roleDialogOpen} onOpenChange={setRoleDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{getText('merchant.customers.editRoleTitle', 'Edit User Role')}</DialogTitle>
              <DialogDescription>
                {getText('merchant.customers.editRoleDesc', 'Change the role for')} {roleEditingUser?.username || roleEditingUser?.email}
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                <Label>{getText('merchant.customers.selectRole', 'Select Role')}</Label>
                <Select value={newRole} onValueChange={(v: 'USER' | 'TENANT_ADMIN') => setNewRole(v)}>
                  ...
                  <SelectContent>
                    <SelectItem value="USER">{getText('merchant.customers.roleUser', 'User')}</SelectItem>
                    <SelectItem value="TENANT_ADMIN">{getText('merchant.customers.roleTenantAdmin', 'Tenant Admin')}</SelectItem>
                  </SelectContent>
                </Select>
                <p className="text-xs text-gray-500">
                  {newRole === 'TENANT_ADMIN'
                    ? getText('merchant.customers.adminRoleDesc', 'Tenant Admin has full access to manage the store.')
                    : getText('merchant.customers.userRoleDesc', 'Regular user with standard customer permissions.')}
                </p>
              </div>
            </div>
            <DialogFooter>
              ...
              >
                {updateRoleMutation.isPending ? getText('merchant.customers.saving', 'Saving...') : getText('merchant.customers.saveRole', 'Save Role')}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
