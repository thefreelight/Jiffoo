                ...
                <div className="mt-6 grid gap-4">
                  {effectiveOrder.shipments.map((shipment) => (
                    <div key={shipment.id} className="rounded-[var(--bokmoo-radius-md)] border border-[var(--bokmoo-line)] bg-[var(--bokmoo-bg)] p-4">
                      ...
                      {shipment.events?.length ? <ol className="mt-5 border-l border-[var(--bokmoo-line-strong)] pl-4">{shipment.events.map((event: { status: string; description?: string | null; occurredAt: string }, eventIndex: number) => <li key={`${event.status}-${event.occurredAt}-${eventIndex}`} className="relative pb-4 last:pb-0"><span className="absolute -left-[21px] top-1 h-2.5 w-2.5 rounded-full bg-[var(--bokmoo-primary)] ring-4 ring-[var(--bokmoo-bg)]" /><div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between"><p className="text-xs font-semibold text-[var(--bokmoo-ink)]">{event.status.replace(/_/g, ' ')}</p><time className="text-[11px] text-[var(--bokmoo-copy-soft)]" dateTime={event.occurredAt}>{new Date(event.occurredAt).toLocaleString()}</time></div>{event.description ? <p className="mt-1 text-xs leading-5 text-[var(--bokmoo-copy)]">{event.description}</p> : null}</li>)}</ol> : null}
                    </div>
                  ))}
                </div>
              </section>
