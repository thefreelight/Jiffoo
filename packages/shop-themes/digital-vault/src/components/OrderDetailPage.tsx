    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[var(--vault-bg)] px-4">
        <div className="rounded-[var(--vault-radius-lg)] border border-[var(--vault-line)] bg-[var(--vault-surface)] p-10 text-center shadow-[var(--vault-shadow)]">
          <h1 className="text-3xl font-black tracking-[-0.04em] text-[var(--vault-ink)]">Order not found</h1>
          {handleBack ? (
            <button
              onClick={handleBack}
      ...
      <div className="mx-auto max-w-[1280px]">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,0.95fr)_minmax(20rem,0.7fr)]">
          <section className="space-y-4">
            <div className="rounded-[var(--vault-radius-lg)] border border-[var(--vault-line)] bg-[var(--vault-surface)] p-6 shadow-[var(--vault-shadow)] sm:p-8">
              <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                <div>
                  ...
                  <h1 className="mt-5 text-[clamp(2.2rem,4vw,4rem)] font-black tracking-[-0.05em] text-[var(--vault-ink)]">
                ...
                {onCancelOrder && ['PENDING', 'PAID'].includes(order.status.toUpperCase()) ? (
                  <button
                    ...
                    className="inline-flex min-h-12 items-center justify-center rounded-full border border-[var(--vault-line)] bg-[var(--vault-bg)] px-5 text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--vault-copy)]"
                  >
                    Cancel order
                  </button>
                ) : null}
            ...
            {order.items.map((item) => {
              ...
              return (
                <article
                  key={item.id}
                  className="rounded-[var(--vault-radius-lg)] border border-[var(--vault-line)] bg-[var(--vault-surface)] p-6 shadow-[var(--vault-shadow)] sm:p-8"
                >
                  ...
                  {hasContent ? (
                    ...
                    <div className="mt-6 rounded-[var(--vault-radius-md)] border border-dashed border-[var(--vault-line)] bg-[var(--vault-bg)] p-5 text-sm leading-6 text-[var(--vault-copy)]">
                      This order is still waiting for fulfillment artifacts. Refresh the page or check again from the order archive shortly.
                    </div>
                  )}
                </article>
              );
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm leading-6 text-[var(--vault-copy)]">
                    Keep a copy of any sensitive credentials in your own secure vault after claiming the order.
                  </p>
                </div>
              </div>
            </div>
