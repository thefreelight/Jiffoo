/**
 * Bestsellers Page — TravelPass Design
 * Top-selling eSIM packages with FA icons and inline product cards.
 */
...
export const BestsellersPage = React.memo(function BestsellersPage({
  products,
  isLoading,
  totalProducts,
  currentPage,
  ...
  const getProductImage = (product: (typeof products)[0]): string => {
    ...
    return (img as { url?: string }).url ?? '/placeholder-product.svg';
  };
      ...
      <section className="bg-blue-600 pt-28 pb-12">
        <div className="container mx-auto px-4 text-center">
          <h1 className="text-3xl md:text-4xl font-bold text-white">
            ...
            {getText('travelpass.bestsellers.title', 'Bestselling eSIM Packages')}
          </h1>
          <p className="mt-3 text-blue-100 text-lg max-w-2xl mx-auto">
            {getText('travelpass.bestsellers.subtitle', 'Our most popular travel connectivity plans trusted by thousands')}
          </p>
        </div>
      </section>
                  ...
                  <div key={product.id} className="bg-white rounded-lg shadow-sm overflow-hidden hover:-translate-y-[3px] hover:shadow-md transition-all duration-200">
                    <div className="relative h-48 bg-gray-100">
                      ...
                      {idx < 3 && (
                        <span className="absolute top-3 left-3 bg-yellow-500 text-white text-xs font-bold px-2 py-1 rounded">
                          <i className="fas fa-crown mr-1" />#{idx + 1}
                        </span>
                      )}
                      {hasDiscount && (
                    ...
                    <div className="p-5">
                      ...
                      <div className="space-y-2 mb-4 text-sm text-gray-600">
                        ...
                        <div className="flex items-center gap-2"><i className="fas fa-calendar-alt text-blue-600 w-4 text-center" /><span>Flexible duration</span></div>
                      </div>
