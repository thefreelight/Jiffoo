        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-50">
            <div className="text-center max-w-md px-4">
                ...
                <h2 className="text-3xl font-bold text-gray-900 mb-4">
                    {getText(t, 'travelpass.notFound.title', 'Page Not Found')}
                </h2>
                <p className="text-gray-500 mb-8">
                    {message || getText(t, 'travelpass.notFound.message', `Sorry, we couldn't find the page${route ? ` "${route}"` : ''} you're looking for.`)}
                </p>
                <button onClick={onGoHome} className="bg-blue-600 hover:bg-blue-700 text-white py-3 px-6 rounded-md transition">
                    {getText(t, 'travelpass.notFound.backHome', 'Back to Home')}
                </button>
            </div>
        </div>
    );
