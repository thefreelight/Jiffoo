...
export function NewArrivalsPage({ products, isLoading, totalProducts, currentPage, totalPages, sortBy, config, onSortChange, onPageChange, onAddToCart, onProductClick }: NewArrivalsPageProps) {
    ...
    return (
      <div className="flex justify-center items-center min-h-screen bg-[#0f0f0f] text-[#bdbdbd] font-mono">
        <div className="text-center">
          ...
          <p className="text-[10px] text-[#bdbdbd] uppercase tracking-widest">Querying Recent Mappings...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f] text-[#bdbdbd] font-mono">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center">
            <div className="inline-flex items-center gap-2 bg-[#1c1c1c] border border-[#2a2a2a] text-[#eaeaea] px-3 py-1 text-[10px] font-bold mb-4 uppercase tracking-widest">
              <Zap className="h-3 w-3 text-[#bdbdbd]" />
              <span>NEW_ALLOCATION_RANGE</span>
            </div>
            <h1 className="text-3xl lg:text-5xl font-bold text-[#eaeaea] mb-4 uppercase tracking-tighter">LATEST_DEPLOYMENTS</h1>
            <p className="text-xs text-[#bdbdbd] max-w-2xl mx-auto uppercase tracking-widest">Scan the most recently integrated network resources available for provisioning.</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-4 border-b border-[#2a2a2a] bg-[#0f0f0f]/90 backdrop-blur-md sticky top-0 z-20">
        <div className="container mx-auto px-4">
          <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
            <div className="text-[10px] text-[#bdbdbd] uppercase tracking-widest">DETECTED <span className="font-bold text-[#eaeaea] opacity-100">{totalProducts}</span> RECENT_NODES</div>
            <div className="relative group">
              <select value={sortBy} onChange={(e) => onSortChange(e.target.value)} className={cn('px-4 py-2 rounded-none border border-[#2a2a2a] text-[10px] bg-[#141414] text-[#eaeaea] uppercase tracking-widest appearance-none pr-8', 'focus:outline-none focus:border-[var(--c-eae)] hover:bg-[#1c1c1c] transition-colors relative z-10 cursor-pointer')}>
                <option value="newest">SORT: DESC_TIMESTAMPS</option>
                <option value="oldest">SORT: ASC_TIMESTAMPS</option>
                <option value="price-high">SORT: COST_DESC</option>
                <option value="price-low">SORT: COST_ASC</option>
                <option value="rating">SORT: DIAGNOSTIC_PASS</option>
              </select>
      ...
      {/* Call to Action */}
      <section className="py-16 md:py-24 border-t border-[#2a2a2a] bg-[#141414] relative">
        <div className="absolute top-0 right-0 w-full h-px bg-gradient-to-r from-transparent via-[var(--c-eae)] to-transparent"></div>
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center">
            <h2 className="text-lg font-bold text-[#eaeaea] mb-4 tracking-widest uppercase">SCAN_EXTENDED_NETWORK?</h2>
            <p className="text-[10px] text-[#bdbdbd] mb-8 max-w-2xl mx-auto uppercase tracking-widest">Expand search parameters targeting older infrastructure configurations.</p>
            <Button variant="outline" className="border-[#2a2a2a] text-[10px] uppercase tracking-widest hover:bg-[var(--c-fff)] hover:text-[var(--c-000)] py-4 px-8 rounded-none">
              EXPAND_SEARCH_RADIUS <ArrowRight className="ml-2 h-4 w-4" />
            </Button>
          </motion.div>
        </div>
      </section>
