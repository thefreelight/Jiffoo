...
export function ProductCard({
  ...
  if (viewMode === 'list') {
    return (
      <div
        onClick={onClick}
        className="group flex flex-col md:flex-row gap-6 p-4 bg-muted border border-border hover:border-[var(--c-eae)] transition-all duration-300 cursor-pointer w-full relative"
      >
        ...
        {/* Image */}
        <div className="relative w-full md:w-48 h-48 md:h-auto flex-shrink-0 bg-background border border-border overflow-hidden">
          <img
            src={imageUrl}
            alt={product.name}
          ...
          />
          {isOutOfStock && (
            <div className="absolute inset-0 bg-background flex items-center justify-center flex-col">
              <span className="text-muted-foreground font-mono text-xs uppercase tracking-widest">[ UNAVAILABLE ]</span>
            </div>
          )}
        </div>
        ...
        <div className="flex-1 flex flex-col py-2">
          ...
          {product.description && (
            <p className="text-sm text-muted-foreground font-light mt-2 line-clamp-2 max-w-2xl">
              {product.description}
            </p>
          )}
          ...
          <div className="mt-auto pt-4 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div className="flex items-baseline gap-3">
              <span className="text-2xl font-mono font-bold text-foreground">${product.price.toFixed(2)}</span>
              {hasDiscount && (
  ...
  return (
    <div
      className="group h-full flex flex-col bg-muted border border-border hover:border-[var(--c-eae)] transition-all duration-300 cursor-pointer overflow-hidden p-3 relative"
      onClick={onClick}
    >
      <div className="absolute top-0 left-0 w-2 h-2 border-t border-l border-[var(--c-eae)] opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
      <div className="absolute bottom-0 right-0 w-2 h-2 border-b border-r border-[var(--c-eae)] opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
      ...
      <div className="relative w-full h-56 flex-shrink-0 bg-background border border-border overflow-hidden mb-4">
        <div className="absolute inset-0 border border-border pointer-events-none z-10 mix-blend-overlay"></div>
        ...
        <div className="absolute inset-0 bg-background group-hover:opacity-0 transition-opacity duration-300"></div>

        {/* Badges */}
        <div className="absolute top-0 w-full flex justify-between p-2 z-20">
          <div className="font-mono text-[10px] bg-background/80 backdrop-blur-sm border border-border px-2 py-1 text-muted-foreground uppercase">
      ...
      <div className="flex-1 flex flex-col px-1 pb-1">
        ...
        <h3 className="font-bold text-lg text-foreground group-hover:text-muted-foreground transition-colors line-clamp-2 uppercase leading-tight mb-3">
          {product.name}
        </h3>

        {/* Price - push to bottom */}
        <div className="mt-auto border-t border-border pt-3 mb-4">
        ...
        <button
          ...
          className={cn(
            ...
            isOutOfStock
              ? "border-border text-muted-foreground cursor-not-allowed bg-accent"
